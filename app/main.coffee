    ...
    build = =>
      ...
      fs.writeFile "#{@options.output}/_swagger-ui.coffee", appContents.join('\n\n'), 'utf8', (err) =>
        ...
        exec "coffee --compile #{@options.output}/_swagger-ui.coffee", (err, stdout, stderr) =>
          ...
          exec "cat lib/doc.js #{@options.output}/_swagger-ui-templates.js #{@options.output}/_swagger-ui.js > #{@options.output}/js/swagger-ui.js", (err, stdout, stderr) =>
            throw err if err
            fs.unlink "#{@options.output}/_swagger-ui.js"
            fs.unlink "#{@options.output}/_swagger-ui-templates.js"
            console.log '   : Minifying all...'
            exec 'java -jar "./bin/yuicompressor-2.4.7.jar" --type js -o ' + @options.output + '/js/swagger-ui.min.js ' + @options.output + '/js/swagger-ui.js', (err, stdout, stderr) =>
              throw err if err
              pack()
